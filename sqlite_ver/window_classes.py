...
class Calendar(tk.Tk):
    ...
    def waiting_for_events(self):
        ...
        time_rn = time.time()
        for row in data:
            current_date = time_str_to_float(row[1])
            if time_rn >= current_date:
...
class StartWindow(ttk.Frame):
    def __init__(self, master: Calendar):
        ...
        for index, distinct_date in enumerate(unique_dates):
            ...
            cal_cur.execute("SELECT reminder_id, reminder_date, reminder_desc FROM reminders "
                            f"WHERE reminder_date LIKE '_____{distinct_date}%'"
                            "ORDER BY reminder_date")
            data = cal_cur.fetchall()
            am_data = []
            pm_data = []
            for row in data:
                if row[1][-2:] == "am":
                    ...
                    pm_data.append(row)
            data = am_data + pm_data
            for x, row in enumerate(data):
                data[x] = (row[0], row[1][-7:], row[2])
...
class ScheduleWindow(ttk.Frame):
    def __init__(self, master: Calendar, current_date):
        ...
        for i, reminder in enumerate(data):
            ...
            reminder_hour = reminder_date[-7:-5]
            reminder_minutes = reminder_date[-4:-2]
            ...
            self.row_container.append((time_label, event_label, event_time_hour, event_time_min, event_time_day,
                                       delete_button, don_del_pass, del_when_pass, line_seperator, event_desc,
                                       reminder_id))
